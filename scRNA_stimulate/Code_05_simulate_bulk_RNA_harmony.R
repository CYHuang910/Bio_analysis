...
rm(a, i, ref_exp_full, unique_table, ref_metadata)
gc()
...
cat("\ncheck matrix negative value: ", sum(apply( ref_exp_edit , 2 , function(x) sum ( x < 0 ) )), "\n")
ref_exp_edit[1:3, 1:2]

#剩下和 CGL2 病人交集的基因
cgl2_h_gene <- read.table('/staging/biology/u8324386/HCC_scRNA/cgl2_harmony_gene.txt')
cgl2_h_gene <- unlist(cgl2_h_gene, use.names = FALSE)
ref_exp_edit <- ref_exp_edit[cgl2_h_gene, ]
cat("\nref_exp_edit remain cgl2 gene: ", dim(ref_exp_edit))
ref_exp_edit[1:3,1:2]

rm(ref_scrna, exp_min)
gc()
...
#one_donor_sample_num <- 8000/nrow(donor_table)
...
for (i in c(1:nrow(new_wall))) {
  row.names(new_wall)[i] <- paste("Simulate_RNA_after_scRNA_harmony_", i, sep = "")
}
...
for (i in c(1:ncol(simulate_rna_with_batch))) {
  colnames(simulate_rna_with_batch)[i] <- paste("Simulate_RNA_after_scRNA_harmony_", i, sep = "")
}
...
#Save SCT data
write.csv(wall_table, paste(args.o, "-1.Simulate_RNA_after_harmony_filter_wall_table.csv", sep=""), row.names = T)
write.csv(pick_cell_id, paste(args.o, "-2.Simulate_RNA_after_harmony_filter_cell_id_table.csv", sep=""), row.names = T)
write.csv(meta_table, paste(args.o, "-3.Simulate_RNA_after_harmony_filter_metadata.csv", sep=""),row.names = F)
write.csv(simulate_rna_with_batch, paste(args.o, "_Simulate_RNA_after_harmony_filter.csv", sep=""), row.names = T)
...
ggsave(paste(umap_wd, "_Simulate_RNA_after_harmony_filter_umap.pdf", sep=""),
       p, width=6, height=6, units="in", scale=1.5)
...
p_x = ggplot(dfx, aes(x, y)) +
  ...
  theme(plot.title = element_text(hjust = 0.5), legend.text=element_text(size=10),
        plot.tag.position = c(0.9, 0.97))
ggsave(paste(umap_wd, "_Simulate_RNA_lognor_after_harmony_filter_umap.pdf", sep=""),
       p_x, width=6, height=6, units="in", scale=1.5)
